...
* You can [enable Function Invocation Service logs](https://docs.oracle.com/en-us/iaas/Content/Functions/Tasks/functionsexportingfunctionlogfiles.htm) to see the logs generated by the Function.
Setting `LOGGING_LEVEL = DEBUG` causes the Function to emit debugging logs.  Please note that `debug` mode generates a _lot_ of logs. We recommend that you set `LOGGING_LEVEL = INFO` unless you are actively debugging.
